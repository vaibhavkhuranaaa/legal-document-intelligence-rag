...
- Full PDF/image ingestion pipeline built and tested against Azure Document
  ...
  reporting. See `docs/architecture.md` for the current architecture summary.

## Phase 1.5: Azure Environment and Live Validation
...
- The current architecture record (`docs/architecture.md`) defines the
  implemented system and its evidence boundaries.
...
The priority roadmap includes parser v2 (outline state
...
Status: **Complete. Promoted through the Phase 6.1 release gate**
...
Status: **Complete. r3 promoted and migrated to usage-based Search**
...
- The App Service now serves `legal-rag-chunks-r3` after a restart reloaded its
  ...
  official SEC filing.
- The active r3 index was copied to Serverless Developer with exact metadata
  parity and identical hybrid top-result parity. The live evidence response was
  byte-identical before and after cutover, and a generated answer retained its
  source anchors. The prior Basic service, r2 index, and legacy index were then
  deleted to remove fixed Search spend. Serverless Developer has no SLA and is
  not recommended for production workloads; recovery requires a controlled
  rebuild from the approved corpus artifacts.
