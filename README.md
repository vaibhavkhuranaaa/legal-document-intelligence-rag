...
| --- | --- | --- | --- | --- | --- |
| Azure | Flask and Gunicorn on Azure App Service with managed identity, Azure AI Search, and Blob Storage | live | anonymous | 2026-08-18T05:13:11Z | No |

## Technology decisions and trade-offs

| Decision | Why | Alternative | Trade-off |
| --- | --- | --- | --- |
| Azure AI Search with Azure OpenAI | Managed retrieval, identity, and model services support a deployable Azure reference architecture. | Local Chroma and API-key based model access | Serverless Search removes fixed idle compute cost, but the preview has no SLA and future usage and storage charges require monitoring. |
...
| --- | --- | --- |
| Azure AI Search and Azure OpenAI | Owner-managed Azure services with a promoted 3,055-chunk public index. | Search compute is usage-based and currently unbilled during preview; model calls remain usage-based, and both require cost monitoring. |
...
| `docs/` | Architecture, scope, deployment, decision, and evaluation records. |
...
| Tests | `PYTHONPATH=src .venv/bin/python -m pytest -q` | [test.repository-suite](tests/) |
...
| [`test.repository-suite`](tests/) | test | The repository test suite passed locally with explicit source import resolution. | Run `PYTHONPATH=src .venv/bin/python -m pytest -q` from the repository root. | 179 passed |
